...
export default function App() {
  ...
  const handlePlayerApply = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast("✓ Profile saved to Live Cloud Database!", "success");
      } else {
        showToast("⚠️ Backend syncing, try again.", "error");
      }
    } catch (error) {
      showToast("⏳ Backend is waking up, wait 10 seconds...", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUmpireRegister = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast("✓ Registered as Official Umpire!", "success");
      }
    } catch (error) {
      showToast("⏳ Backend is waking up, wait 10 seconds...", "error");
    } finally {
  ...
  return (
    <div style={{ backgroundColor: '#0b0f19', color: '#f3f4f6', minHeight: '100vh', fontFamily: 'sans-serif', paddingBottom: '40px', position: 'relative' }}>
      
      {/* PREMIUM IN-APP NOTIFICATION TOAST */}
      {toast.show && (
        <div style={{ position: 'fixed', top: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: toast.type === 'success' ? '#10b981' : '#ef4444', color: '#fff', padding: '12px 24px', borderRadius: '8px', fontWeight: 'bold', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.5)', transition: 'all 0.3s ease' }}>
          {toast.message}
        </div>
      )}
      ...
      <nav style={{ position: 'sticky', top: 0, backdropFilter: 'blur(12px)', backgroundColor: 'rgba(11, 15, 25, 0.8)', borderBottom: '1px solid #1f2937', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', gap: '20px', fontSize: '0.9rem', fontWeight: '500' }}>
          <span style={{ color: '#9ca3af', cursor: 'pointer' }} onClick={() => showToast('Matches view loading...', 'success')}>Matches</span>
          <span style={{ color: '#9ca3af', cursor: 'pointer' }} onClick={() => showToast('Tournaments view loading...', 'success')}>Tournaments</span>
          <span style={{ color: '#10b981', cursor: 'pointer' }}>Hiring</span>
        </div>
      </nav>
      ...
      <div style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: '12px', maxWidth: '360px', margin: '-20px auto 40px auto', overflow: 'hidden', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.3)', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ borderTop: '1px solid #1f2937', padding: '15px 20px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <button onClick={handlePlayerApply} disabled={loading || appliedPlayer} style={{ backgroundColor: appliedPlayer ? '#065f46' : '#1f2937', color: appliedPlayer ? '#10b981' : '#fff', width: '100%', border: '1px solid #374151', padding: '12px', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', cursor: 'pointer' }}>
            {loading ? 'Processing...' : appliedPlayer ? '✓ Spot Application Sent' : 'Apply for Player Spot'}
          </button>
          <button onClick={handleUmpireRegister} disabled={loading || appliedUmpire} style={{ backgroundColor: appliedUmpire ? '#065f46' : 'transparent', color: appliedUmpire ? '#10b981' : '#fff', width: '100%', border: '1px solid #10b981', padding: '12px', borderRadius: '8px', fontWeight: '600', fontSize: '0.85rem', cursor: 'pointer' }}>
      ...
      <div style={{ padding: '0 20px', maxWidth: '360px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: '#111827', border: '1px solid #1f2937', borderRadius: '10px', padding: '20px', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #1f2937', paddingTop: '12px', marginTop: '12px' }}>
            ...
            <button onClick={() => showToast('Job Application Portal Loading...', 'success')} style={{ backgroundColor: '#10b981', color: '#0b0f19', border: 'none', padding: '8px 14px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: '700', cursor: 'pointer' }}>Apply Now</button>
          </div>
        </div>
      </div>
